...
class Maze {     
    ...
    private:
        ...
        int med_rows;
        void PutBeginEnd(){
            board[(int)startPosition.x][(int)startPosition.y] = agent1; // Put the Start
            board[(int)endPosition.x][(int)endPosition.y] = agent2; // Put the End
        }

        void ReserveMemory(){
            board = new char* [columns]; // Maze size with M columns

            for (int i = 0; i < columns; i++) {
                board[i] = new char [rows]; // Maze size with N columns
            }
        }
