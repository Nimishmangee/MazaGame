...
#define MED_COLUMNS 6// Size of the Board
#define MED_ROWS 6
#define SIZE_SQUARE_SMALL 3
#define WIDTH 800  //Size of the Window
#define HEIGHT 800

int TIME = 60; // Game Time
int TIME_PLAYER_OFF = 2; // Time the player disappers
int time_show = TIME;  // Assistant for playing time
int time_show_shooted = TIME_PLAYER_OFF; // Assist by a shot player

int flagExit = 0; // Flag to leave the Game
int flagShooted = 0; // Flag to know if they fired
int endGame =0; // Flag to end the Game
...
long last_t=0;
#define PI 3.1416
int index_path = 0; // Index for the dfs to take next move

Maze maze(MED_COLUMNS, MED_ROWS);// Create the Maze
Walls wall(maze.getNumWalls()); // Add the walls
Tank agent1(maze.agent1); // Agent1 is the Player
Tank agent2(maze.agent2); // Agent2 is AI

dfs intell(COLUMNS, ROWS, maze.board); // Perform dfs to find the path
//-----------------------------------------------
//                FUNCTIONS
//-----------------------------------------------
void randomMove();
...
int main(int argc,char *argv[])
{
    ...
    LoadTexture((char *)"textures/download.jpeg",64);//outer wall
    
    glBindTexture(GL_TEXTURE_2D,1);
    LoadTexture((char *)"textures/download.jpeg",64);//upper wall
    
    glBindTexture(GL_TEXTURE_2D,2);
    LoadTexture((char *)"textures/download.jpeg",64);//inner wall
    
    glBindTexture(GL_TEXTURE_2D,3);
    LoadTexture((char *)"textures/images.jpeg",64);//floor
...
void display_text(string s, int color=0){
    ...
    if(color ==0)(time_show<= 10) ? glColor3f(1.0, 0.0, 0.0): glColor3f(1.0, 1.0, 1.0);
...
void display() {
    ...
    glClearColor(0.28,0.48,0.17,1.0);
